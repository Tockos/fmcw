...
%distance measurement to file
    ...
    for ind=1:length(R_points)
        ...
        target.v=v_stat;
        target.fd = get_doppler(target.v);
        target.tau0 = get_tau0(target.r0);
        target.Theta = get_theta(target);

        [r_meas] = signalproc(target);
...
%velocy measurement to file
    ...
    for ind=1:length(V_points)
        ...
        [r_meas, v_meas] = signalproc(target);
        fprintf(fileID,'%15d %30d\n', V_points(ind), v_meas);
